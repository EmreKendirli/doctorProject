...
const PropertyMediaUploader = ({
  ...
  useEffect(() => {
    if (stepValue === 4 && !propertySelectedImgs.length) {
      if(crudFrom.formValues.advertPhoto){
        setPropertySelectedImgs(crudFrom.formValues.advertPhoto)
      }else if(values?.advertPhoto?.length){
